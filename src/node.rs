...
impl Node {
    ...
    pub fn update_height_size(&mut self) {
        let lh = self.left.as_ref().map_or_else(|| 0, |n| n.height);
        let rh = self.right.as_ref().map_or_else(|| 0, |n| n.height);

        self.height = 1 + std::cmp::max(lh, rh);
        println!("lh {lh} rh {rh} {}", self.height);
        let l_size = self.left.as_ref().map_or_else(|| 0, |n| n.size);
        let r_size = self.right.as_ref().map_or_else(|| 0, |n| n.size);
    ...
    pub fn balance_factor(&self) -> i32 {
        let left_height = self.left.as_ref().map_or_else(|| 0, |n| n.height) as i32;
        let right_height = self.right.as_ref().map_or_else(|| 0, |n| n.height) as i32;
